...
export class MB1FloorPlan {
  ...
  ngOnInit(){
    ...
    document.addEventListener('click', (res: any) =>{
      ...
      if(ele.includes('wc') || ele.includes('mb1') || ele.includes('down') || ele.includes('up') || ele.includes('elevator') || ele.includes('out')){
        this.resetNav();
        this.initNav(ele);
      }
    });
    ...
    this.events.subscribe('init-new-path', data => {
      ...
      this.setMarker(p);
      this.indoorDirectionsService.computePathHelper(this.sourceID, this.destID);
    })
  }
  ...
  public initNav(name: string){
    ...
    if(name.includes('mb')){
      ...
      point = this.interestPoints.filter(point => point.id === 'escalators-down')[0];
